...
public class _10_TypeCasting {
	...
	public static void main(String[] args) {
		...
		// long은 8byte고 float은 4byte로 long리 byte 크기는 더 크지만 float가 더 세밀한 표현범위를 갖고 있기 때문에
		// lNum1이 float 타입으로 변환된다.
		float  fNum1 = lNum1;
		
		// 3. 강제형변환1: byte 크기가 큰 타입에서 작은 타입으로 변화할 때는 타입을 명시해줘야 한다.
		int iNum2 = 60;
		
		// int 타입인 iNum2가 short 타입보다 byte 크기가 크기 때문에 강제로 short 타입으로 변환된다.
		...
		// 5. 연산에서의 형변환
		// 형변환순서에 따라서 연산중에 byte 크기가 더 큰 타입이나 좀 더 세밀한 표현점위를 갖는 타입으로 자동 형변환이 일어난다.
