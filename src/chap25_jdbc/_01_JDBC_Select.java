...
public class _01_JDBC_Select {
	...
	public static void main(String[] args) {
		...
		try {
			// 위에서 지정한 Database 드라이버 로드
			Class.forName(JDBC_DRIVER);

			// DriverManager 클래스를 통해 DB 연결 객체 얻기
			...
			// Statement 객체로 쿼리를 실행시키고 결과를 ResultSet에 담아준다
			...
			while (rs.next()) {
				...
				// 문자열 getString 메소드, 정수형은 getInteger, 실수형은 getDouble
			...
			// 사용완료된 Connection, Statement, ResultSet 객체 닫기
			if (rs != null) {
				try {
					rs.close();
				} catch (SQLException se) {
