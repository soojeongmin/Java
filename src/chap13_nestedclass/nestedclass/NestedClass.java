...
// 중첩클래스는 클래스 안에 다른 클래스를 하나 더 정의하는것
...
public class NestedClass {
	...
	// 같은 클래스 내에 있어서 private이여도 접근 가능
	private static int sub(int a, int b) {
		return a - b;
	}
	
	// 5. 정적 중첩 클래스(스태틱 중첩 클래스)
	// 외부클래스의 static 키워드로 생성도니 변수와 메소드에만 접근할 수 있다.
	// 중첩 클래스 자체가 static으로 컴파일러가 메모리에 바로 올리기 때문에
	// 외부클래스의 멤버변수나 인스턴스 메소드는 객체가 생성도니 후에 사용할 수 있기 때문이다.
	...
	public static class Mul {
		...
		public int mul(int a, int b) {
			// 외부 클래스의 블래스 변수 및 클래스 메소드에 접근
			num3 = 10;
			a = sub(num3, 5);
			return a * b;
		}
		
		// 5-4. 정척 중첩 클래스의 클래스메소드
