...
public class _01_ArrayList {
	...
	public static void main(String[] args) {
		...
		// 1. List 객체 생성
		// List는 인터페이스라 객체를 생성할 수 없구
		// List를 상속받아 구현된 ArrayList, LinkedList 등의 객체를 사용한다.
		// List를 생성할 때는 제네릭 타입의 엘레멘트 타입을 지정해야 하는데
		// 지정하지 않고 List를 생성하면 엘레멘트의 타입이 자동으로 Object로 지정된다.
		List list1 = new ArrayList();
		List<Object> list2 = new ArrayList<>();
		
		// 엘레멘트의 타입이 Object로 저장되었기 때문에
		...
		// 2. Java17 버전 이전에서는 ArrayList생성자에서도 엘레멘트의 제네릭타입을 지정해야 했지만
		// Java17버전부터는 제네릭 타입을 명시하지 않아도 된다.
		List<String> strList = new ArrayList<>();
		
		// 자바17이전버전
		List<String> strList2 = new ArrayList<String>();
		
		// 3. 제네릭 타입에 기본타입은 지정될 수 잆다.
		...
		// 4. 포함여부 확인
		// containsAll(Collection c): 매개변수로 전달된 Collection 객체가 모두 포함되었는지 확인
		// contains(Object c): 매개변수로 전달된 객체가 목록에 존재하는지 확인
		...
		// addAll(Collection c): 매개변수로 전달받은 컬렉션 객체의 데이터를 목록 마지막에 모두 추가
		intList2.add(10);
		intList2.add(13);
