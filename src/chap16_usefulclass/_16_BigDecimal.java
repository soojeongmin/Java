...
public class _16_BigDecimal {
	...
	public static void main(String[] args) {
		// 1. BigDecimal 객체 생성
		// 1-1. 생성자를 통한 객체 생성
		BigDecimal bigDecimal1 = new BigDecimal(1.99999999999999999999999999999999999);
		...
		// 반올림 모드는 MathContext 객체를 통해서 친행되는데
		// MathContext의 생성자를 통해 객체를 생성할 때 매개변수로
		// MathContect(int 자리수, 반올림모드(RoundingMode enum 사용))를 전달한다.
		// 자리수는 정수자리까지 포함된 자리수이다.
		System.out.println(bigDecimal2.divide(bigDecimal1, new MathContext(4, RoundingMode.HALF_UP)));
		
		// 3. setScale: 소수점 자리수 재설정 
		// setScale(int 소수점 자리수, 반올림보드)
