...
public class _07_WildCard {
	...
	// 타입매개변수를 지정하지 않고 바로 사용가능한 제네릭 타입을 와일드카드<?>라고 한다.
	...
	// 서브클래스 타입으로 사용할 수 있다.
	// public static List<?> createList(List<?> whildCardList) { }
	...
	// 2. 와일드카드<?>도 extends 로 상한제한을 걸 수 있다.
	...
	// <? super 자식클래스>: 자식클래스에게 상속해준 부모클래스와 자식클래스만 메소드의 리턴타입 매개변수로 지정가능하다.
	...
	// 제네릭 타입매개변수<T, K, V, E, ...>는 제네릭 클래스와 제네릭 메소드에 모두 사용 가능하다.
	// 4-2. 제네릭 타입매개변수<T, K, V, E, ...>는 메소드의 정의부나 클래스의 정의부에 타입매개변수를
	// 항상 지정해야 하지만 와일드카드<?> 타입매개변수를 지정 안해도 된다.
	// 4-3. 제네릭 타입매개변수<T, K, V, E, ...>는 단독으로 사용가능(타입매개변수 타입의 변수선언, 리턴타입, 매개변수)
