...
public class _17_CountingMethodEx {
	...
	public static void main(String[] args) {
		...
		HyundaiCar max = hCarList.stream().max(Comparator.comparing(car -> car.getPrice())).orElseThrow();
		HyundaiCar min = hCarList.stream().min(Comparator.comparing(car -> car.getPrice())).orElseThrow();
		max.carInfo();
		min.carInfo();
	}
