...
public class _06_ArraysToStream {
	...
	public static void main(String[] args) {
		...
		// 을 이용한다. of() 메소드도 IntStrea, DoubleStream, LongStream의 of() 메소드 사용
		IntStream intStream1 = Arrays.stream(intArr);
		IntStream intSteram2 = IntStream.of(intArr);
		...
		// String은 클래스이자 참조타입이기때문에 Stream<String> 형태로 변환한다.
		Stream<String> strStream1 = Arrays.stream(strArr);
		Stream<String> strStream2 = Stream.of(strArr);

		// IntStream도 stream이기 때문에
		// stream에서 지원하는 모든 메소드 사용 가능(forEach, map, filter, reduce...)

		// intStream1에서 5 이상 되는 값만 추출하고
		// 추출된 값들에서 4를 곱한 요소로 구성된 새로운 Stream을 만들고
		// 요소들을 하나씩 출력하세요.
		// 배열을 스트림으로 변환해도 파이프라인 구성 가능하다.
		intStream1.filter(num -> num >= 5).map(num -> num * 4).forEach(num -> System.out.println(num));
