...
public class _18_Collect {
	...
	public static void main(String[] args) {
		...
		List<CreditCard> samsungCardList = creditCardList.stream().filter(card -> card.getCompany().equals("삼성카드"))
		...
		// collect(Collectors.toMap(키값을 지정하는 람다식, 벨류를 지정하는 람다식))
		// Map<cardName, limit>
		Map<String, Integer> cardMap = creditCardList.stream().filter(card -> card.getLimit() >= 5000)
				.collect(Collectors.toMap(card -> card.getCardName(), card -> card.getLimit()));
