...
public class _19_GroupingBy {
	...
	public static void main(String[] args) {
		...
		Map<String, List<CreditCard>> companyCardMap = creditCardList.stream()
		...
		// Collectors.groupingBy(키값으로 지정할 람다식, Collectors.averaging(람다식));
		Map<String, Double> companyLimitAvgMap = creditCardList.stream().collect(
				Collectors.groupingBy(card -> card.getCompany(), Collectors.averagingDouble(card -> card.getLimit()))); // 그룹화된
																														// 리스트를
																														// 사용하는
																														// 메소드
		...
		Map<String, Long> cardCountMap = creditCardList.stream().collect(Collectors.groupingBy(card->card.getCompany(), Collectors.counting()));
